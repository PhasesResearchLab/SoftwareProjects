# 相位研究实验室软件项目
该存储库作为[相位研究实验室](https://phaseslab.org)成员正在进行的所有软件项目的索引，从小型和私有项目到大型的开源项目，重点是后者。
...
- 🟢 ✅ [**ESPEI**](https://github.com/PhasesResearchLab/ESPEI) - （[@bocklund](https://github.com/bocklund) 现在在[LLNL](https://www.llnl.gov/)） - **E**xtensible **S**elf-optimizing **P**hase **E**quilibria **I**nfrastructure，是一个用于创建CALPHAD数据库和评估CALPHAD模型不确定性的工具。ESPEI的目的是成为一个用于拟合最先进的CALPHAD类型模型的用户工具，同时也是一个用于开发拟合和不确定性量化方法的研究平台。
  ...
  [![最新](https://img.shields.io/badge/阅读文档-最新-green)](https://espei.org/en/latest/)
  [![PyPI版本](https://badge.fury.io/py/espei.svg)](https://pypi.org/project/espei)

- 🟢 ✅ [**pySIPFENN**](https://github.com/PhasesResearchLab/pySIPFENN) - py（**S**tructure-**I**nformed **P**rediction of **F**ormation **E**nergy using **N**eural **N**etworks）可以轻松预测形成能量（🟢✅），并通过迁移学习调整模型以适应新材料（🟠）和性质（🔴）的小数据集机器学习。
  ...
  [![最新](https://img.shields.io/badge/阅读文档-最新-green)](https://pysipfenn.readthedocs.io/en/latest/)
...
- 🟢 ✅ [**ULTERA-contribute**](https://github.com/PhasesResearchLab/ULTERA-contribute) - 一个带有越来越多嵌入式自动化的模板数据存储库，用于合金数据集处理，包括数据验证和异常检测。目前，为ULTERA [(ultera.org)](https://ultera.org)贡献硬编码，但很快将被推广。
...
- 🟢 🔬 [**fmat**](https://github.com/HUISUN24/feasibility_map) - **F**easibility of **MAT**erials mapper是一种基于CALPHAD的工具，可帮助避免形成不需要的相，并设计加入不同材料的最佳组成路径。它通过预测平衡和非平衡相的形成过程，提供对制造过程中相形成过程的全面理解。
...
- 🟢 🔬 [**PyQAlloy**](https://github.com/PhasesResearchLab/PyQAlloy) - **P**ython工具集用于**Q**uality of **Alloy**s数据，旨在通过多范围检测一些异常模式，促使重新验证，对大型合金数据集进行策划，特别是像HEA/MPEA/CCA这样容易出错的数据集。
  ...
  [![最新](https://img.shields.io/badge/阅读文档-最新-green)](https://pyqalloy.readthedocs.io/en/latest/)
...
- 🟢 🏗 [**heaGAN**](https://github.com/amkrajewski/cGAN_demo) - （与[Reinhart Group](https://sites.psu.edu/reinhartgroup/people/)共同开发） - 用于（1）训练合金设计的代理模型和（2）使用条件**G**enerative **A**dversarial **N**etworks生成新颖的**h**igh **e**ntropy **a**lloys设计的演示工作流程。您可以在云端运行它并下载您训练的模型。
...
- 🤏 🟢 🔬 [**optimade-python-tools-mpdd**](https://github.com/PhasesResearchLab/optimade-python-tools-mpdd) - [**Materials-Consortia/optimade-python-tools**](https://github.com/Materials-Consortia/optimade-python-tools)的分支，由[@ml-evs](https://github.com/ml-evs)调整以满足MPDD和其他非常大的内存IO受限的材料数据库的需求。
  ...
  [![静态徽章](https://img.shields.io/badge/OPTIMADE端点-MPDD-purple)](http://mpddoptimade.phaseslab.com/)


### pqam-dparamhu2021

- 🤏 🟢 ✅ [**pqam-dparamhu2021**](https://github.com/amkrajewski/pqam-dparamhu2021) - 基于Yong-Jie Hu的2021年文献模型（使用R进行优化）的**P**y**QA**lloy兼容**M**odel，已经针对高通量进行了优化并用Python封装。
...
- 🟠/🟢 ✅ **MPDD** ([**server**](https://github.com/PhasesResearchLab/MPDD-server) & [**tools**](https://github.com/PhasesResearchLab/MPDD-OPTIMADE)) - **M**aterial-**P**roperty-**D**escriptor **D**atabase是一个原子级数据处理基础设施，允许分散的特征化（描述符计算）和快速的机器学习模型部署在数百万个DFT松弛配置上。数据通过[**OPTIMADE**](https://github.com/Materials-Consortia/OPTIMADE) API公开提供在[mpddoptimade.phaseslab.com](http://mpddoptimade.phaseslab.com/)（🟢），但高通量API和服务器和客户端的源代码目前保留为内部（🟠）。
...
- 🟠 🏗 [**pqam-dparamkrajewski2023**](https://github.com/amkrajewski/pqam-dparamkrajewski2023) - 基于迁移学习的预测耐火合金本征韧性的工具。
...
- 🔴 ✅ **crystALL** - 一组工具，利用新的SIPFENN特征提取器的速度和MPDD中数百万个结构的预测**cryst**al结构的能力，适用于**ALL**化学成分。例如在以下论文中进行了演示：
...
- 🔴 🔬 [**ULTERA**](https://github.com/PhasesResearchLab/ULTERA) - ULTERA项目中开发的一组软件工具，将逐个发布（例如PyQAlloy 🟢）或保留为内部。
...
- 🟢 [**pymatgen**](https://github.com/amkrajewski/pymatgen)：
  - 2023年：1个增强功能和1个错误修复，均针对_pymatgen.**core** 
  - 2017年：1个增强功能针对_pymatgen.**analysis**和1个错误修复针对_pymatgen.**io** 
- 
...
- 🟢 ✅ [**kawin**](https://github.com/materialsgenomefoundation/kawin) - 用于预测沉淀核化和生长行为的Kampmann-Wagner数值（KWN）模型的Python实现。该软件包与pycalphad耦合，用于进行热力学和动力学计算。[**查看文档**](https://kawin.org/)

  [![GitHub最新提交（由提交者）](https://img.shields.io/github/last-commit/materialsgenomefoundation/kawin?label=最新提交)](https://github.com/materialsgenomefoundation/kawin)
...
- 如果您是**活跃的PRL成员**，默认情况下应该具有对该存储库的写访问权限，并且可以直接进行更改。
- 如果您是**过去的PRL成员**，欢迎您贡献（1）在活跃期间工作的代码到适当的类别（如果您不再主动维护，请使用`Legacy`），以及（2）离开团队后创建的代码到`校友工作`。您可以通过fork存储库并打开pull request来贡献。
- 最简单的贡献方式是通过在浏览器中点击`.`键来打开GitHub开发环境。它适用于任何带有键盘的设备（甚至是iPad！）。通过它，**您可以在不了解git的情况下在不到一分钟的时间内进行简单的贡献！**只需（1）编辑自动打开的README文件中的文本，（2）点击左侧面板上的_Source Control_图标（从上往下数第三个），（3）写下您所做的简短消息，然后（4）点击_Commit&Push_。完成！